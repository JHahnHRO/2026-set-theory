...
\end{lemma}
\begin{remark}
    In a recursion over $A=\IN$, we might define $f(n)$ by any computation $g$ that may depend on some or all $f(0), f(1), \ldots, f(n-1)$, i.e.\ any finite-length sequence of values in $B$.

    The appropriate generalisation to sequences with arbitrary well-ordered index sets would be something like $\bigcup_{S\in A^+\setminus\set{A}}Maps(S\to B)$, i.e.\ certain partial maps $A\to B$.
